...
rule kmer_association:
    ...
    shell:
        ...
        awk -F'\\t' 'NR>1 && $5 == 0 && $6 == 1' {output.assoc_clean} > {output.male_assoc} 2>> {log}
        awk -F'\\t' 'NR>1 && $5 == 1 && $6 == 0' {output.assoc_clean} > {output.female_assoc} 2>> {log}

        # Log counts
        echo "Total associations: $(tail -n +2 {output.assoc_clean} | wc -l)" >> {log}
        echo "Male-specific (F_A=0, F_U=1): $(wc -l < {output.male_assoc})" >> {log}
        echo "Female-specific (F_A=1, F_U=0): $(wc -l < {output.female_assoc})" >> {log}
